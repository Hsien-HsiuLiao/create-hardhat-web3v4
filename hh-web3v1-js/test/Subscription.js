...
describe("Token contract subsciption event test", function () {
  ...
  //we desire and invoke the .send to broadcast our intention to the network , .send({from: deployer}).
            ...
            expect(await getBalance(owner)).to.be.equal(BigInt(ownerBalance) - BigInt(50));
            expect(await getBalance(addr1)).to.be.equal(BigInt(addr1Balance) + BigInt(50));
        ...
        it("Should emit Transfer events", async function () {
            ...
            const transferEventToAddr1Promise = new Promise((resolve) => {
              /*   console.log("transferEventToAddr1Promise:", hardhatToken.events.Transfer({
                ...
                /* var subscriptionWeb3v1 = web3.eth.subscribe('logs', function (error, result) {
                ...
                hardhatToken.events.Transfer({
                   ...
                   // fromBlock: 0
                }, function(error, event){ console.log("event, erro",event, error); })
